...
pobierz_parametry_skalowania <- function(nazwa_skali=NULL, id_testu=NULL, 
  ...
  where = paste0(where," and SA.opis LIKE '",opis_skalowania,"'")
  ...
  zapytanie1 = paste0("
                select  
                SA.id_skali, kolejnosc, id_kryterium, id_pseudokryterium,
  ...
  for(ind in 1:nrow(opisSkalowan)){
    ret[[1]] = tablicaDanych[
                            tablicaDanych[,"skalowanie"] == opisSkalowan[,1], 
                            colnames(tablicaDanych) != "skalowanie"
                            ]
    
    for(k in 2:ncol(opisSkalowan)){
      attr(ret[[1]],colnames(opisSkalowan)[k]) = as.character(opisSkalowan[ind,k])
    }
  }
  
  if(nrow(skale)>1){
    warning("Więcej niż jedna skala przpisana do wyników. 
            Argumentami zwracanej listy są wartości dla pierwszej skali.")
  }
